public class Detector {
    ...
    private Coordinates sizeL; //"нижние границы" диапазона координат на окошке (типа (а,y0,z0) для x=a)
    private Coordinates sizeH; //"верхние границы" (a,b0,c0)
    private Coordinates normal = new Coordinates(); //нормаль, пригодится чтоб отражать
    private int counter = 0; //считает фотончики
    private int mode; //1 - используем абсолютный альфа, 2 - считаем по Френелю todo:добавить значение по-умолчанию
    ...
    Detector(){
        this.sizeL  = new Coordinates();
        this.sizeH  = new Coordinates();
    }
    
    Detector(Coordinates sizeL, Coordinates sizeH, double a, double nExt){
        this.nExt = nExt;
        this.alpha = a;
        this.sizeL = new Coordinates(sizeL.getX(), sizeL.getY(), sizeL.getZ());
        this.sizeH = new Coordinates(sizeH.getX(), sizeH.getY(), sizeH.getZ());
